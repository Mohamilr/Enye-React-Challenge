import React, { useState, useEffect, FC, useContext } from 'react';
import { makeStyles, Theme, createStyles } from '@material-ui/core/styles';
import Paper from '@material-ui/core/Paper';
import InputBase from '@material-ui/core/InputBase';
import TextField from '@material-ui/core/TextField'
import SearchIcon from '@material-ui/icons/Search';
import IconButton from '@material-ui/core/IconButton';
import MenuIcon from '@material-ui/icons/Menu';
import Tooltip from '@material-ui/core/Tooltip';
import { Map, TileLayer, Marker, Popup } from 'react-leaflet';
import Icon from '@material-ui/core/Icon';
import L from 'leaflet';
import { firestore } from '../../auth/firebase.config';
import SearchHistory from '../searchHistory';
import { AuthProvider } from '../../utils/useContext';

const icon = L.icon({
    iconUrl: 'data:image/png;base64,',
    iconSize: [30, 25],
    iconAnchor: [24, 14],

})
const useStyles = makeStyles((theme: Theme) =>
    createStyles({
        container: {
            display: 'grid',
            gridTemplateColumns: '2fr 7fr',
        },
        right: {
            marginTop: '3em'
        },
        root: {
            padding: '2px 4px',
            display: 'flex',
            alignItems: 'center',
            width: 500,
        },
        input: {
            marginLeft: theme.spacing(1),
            flex: 1,
        },
        iconButton: {
            padding: 10,
        },
        divider: {
            height: 28,
            margin: 4,
        },
    }),
);

const AutorizedMap: FC = () => {
    const classes = useStyles();
    const [location, setLocation] = useState<string>('');
    const [lat, setLat] = useState<number>(6.5243793);
    const [lng, setLng] = useState<number>(3.3792057);
    const [searchKey, setSearchKey] = useState<string>('');
    const [searchHistory, setSearchHistory] = useState<string[]>([]);
    const [results, setResults] = useState<any[]>([]);
    const { userId } = useContext(AuthProvider);

    useEffect(() => {
        handlePlace();
        HandleGetSearchHistory();
    }, [results])

    const handleSubmit = (e: any) => {
        e.preventDefault()
        const db = firestore;
        const userRef = db.collection('searchHistory').add({
            searchString: searchKey,
            userId: userId
        });
    }

    const HandleGetSearchHistory = async () => {
        const db = firestore;
         await db.collection('searchHistory').get().then(snapshot => {
            const strings: any = [];
            snapshot.docs.map(data => {
                if(data.data().userId === userId) {
                    strings.push(data.data().searchString)
                }
            })
            setSearchHistory(strings)
        })
        .catch(e => console.error(e))
        
    }

    const handlePlace = async () => {
        const response = await fetch(`https://spott.p.rapidapi.com/places/ip/me?language=en`, {
            method: 'GET',
            headers: {
                "x-rapidapi-host": "spott.p.rapidapi.com",
                "x-rapidapi-key": "2b140728d4msh9fed995ffd67651p16455cjsnd6c6d9e20a66",
            }
        });

        const data = await response.json()
        if (data.length < 1) {
            return console.log('not found')
        }
        // console.log('place', data.name)

        setLocation(data.name)
    }

    const handleLocation = async () => {
        const response = await fetch(`https://google-maps-geocoding.p.rapidapi.com/geocode/json?language=en&address=${location}`, {
            method: 'GET',
            headers: {
                "x-rapidapi-host": "google-maps-geocoding.p.rapidapi.com",
                "x-rapidapi-key": "2b140728d4msh9fed995ffd67651p16455cjsnd6c6d9e20a66"
            }
        });

        const data = await response.json()
        if (data.length < 1) {
            return console.log('not found')
        }
        setLat(data.results[0].geometry.location.lat);
        setLng(data.results[0].geometry.location.lng);
    }

    const handleSearch = async () => {
        try {
            const response = await fetch(`https://places-by-name.p.rapidapi.com/v1/geocode/search?limit=50&lat=${lat}&lang=en&lon=${lng}&type=amenity&text=${searchKey}`, {
                method: 'GET',
                headers: {
                    "x-rapidapi-host": "places-by-name.p.rapidapi.com",
                    "x-rapidapi-key": "2b140728d4msh9fed995ffd67651p16455cjsnd6c6d9e20a66",
                }
            })
            const data = await response.json();
            // console.log('results', data)
            if (!data.features) {
                return console.log('not found')
            }

            // for (let i = 0; i < data.features.length; i++) {
            //     console.log(data.features[i].geometry.coordinates)
            // }
            const pos: any = []
            data.features.map((latlng: any) => {
                // console.log(latlng.geometry.coordinates)
                pos.push(latlng.geometry.coordinates)
            })
            setResults(pos)

            // console.log(pos)
        }
        catch (e) {
            console.error(e)
        }

    }
   
    
    return (
        <div className={classes.container}>
            <div className={classes.right}>
                <Paper component="form" className={classes.root}>
                    <InputBase
                        className={classes.input}
                        placeholder="Hospitals, Pharmacies, Clinics and Medical Offices"
                        value={searchKey}
                        onChange={e => {
                            setSearchKey(e.target.value)
                        }}
                        inputProps={{ 'aria-label': 'search google maps' }}
                    />
                    <Tooltip title="Search">
                    <IconButton type="submit" className={classes.iconButton} aria-label="search" onClick={e => {
                        e.preventDefault();
                        handleLocation();
                        handleSearch();
                        handleSubmit(e);
                    }}>
                        <SearchIcon />
                    </IconButton>
                    </Tooltip>
                </Paper>
                <SearchHistory searchHistory={searchHistory} setSearchKey={setSearchKey} handleSearch={handleSearch}/>
            </div>
            <div >
                <Map style={{ height: '100vh', width: '100%' }} center={[lat, lng]} zoom={13}>
                    <TileLayer
                        attribution='&amp;copy <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
                        url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                    />
                    {results.map((data: number[], index) => (
                        <Marker key={index} marker_index={index} position={[data[1], data[0]]} icon={icon} />
                    )
                    )}
                </Map>
            </div>
        </div>
    );
}

export default AutorizedMap;